...
//    List<BotRelation> findAllByBotNode(BotNode botNode);
    ...
    @Query("MATCH p=(n:Bot)<-[r:BotRelation]->(m:Bot)WHERE n.name={startNode} and m.name={endNode} RETURN p")
    List<BotRelation> findBotRelationByStartAndEndNode(@Param("startNode") String startNode, @Param("endNode") String endNode);
